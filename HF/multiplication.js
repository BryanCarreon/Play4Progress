    ...
    if (userAnswer === correctAnswer) {
        ...
        if (correctAnswers % 5 === 0) {
            ...
            try {
              ...
              const multiplicationProgress = progress.multiplicaiton || {};
              const allMultiplicationComplete = Object.values(multiplicationProgress).every(v => v === "complete");
              if (allMultiplicationComplete && !badges.multiplication_master) {
                badges.multiplicaiton_master = true;
                updated = true;
                alert("➕ You earned the Multiplication Master badge!");
                //store the newly eaned badge in localStorage to sync with home page
                const earned = JSON.parse(localStorage.getItem("earnedBadges") || "{}");
                earned.multiplicaiton_master = true;
                localStorage.setItem("earnedBadges", JSON.stringify(earned));
              }
